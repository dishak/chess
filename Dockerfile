...
FROM node:20-alpine as prisma_img
WORKDIR /root
...
WORKDIR /root
COPY --from=prisma_img /root ./
COPY ./apps/backend/package.json ./apps/backend/
COPY ./apps/backend/package-lock.json ./apps/backend/
RUN npm install
COPY ./apps/backend ./apps/backend/
CMD ["sh", "-c", "cd ./packages/db && npm run db:dev && while ! npm run db:dev; do echo 'Waiting for npm run db:seed...'; sleep 1; done && cd ../.. && cd ./apps/backend && npm run dev"]
...
CMD ["sh", "-c", "cd ./packages/db && npm run db:dev && cd ../.. && cd ./apps/ws && npm run dev"]
